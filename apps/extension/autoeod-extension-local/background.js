...
async function R(e) {
  const t = await D(), n = await t.get("sync_queue", e.id);
...
async function re() {
  const t = await (await D()).getAll("sync_queue"), n = Date.now();
  return t.filter((r) => r.nextRetryAt <= n);
}
async function x(e) {
  await (await D()).delete("sync_queue", e);
}
async function oe(e, t, n) {
  const r = await D(), o = await r.get("sync_queue", e);
...
chrome.runtime.onMessage.addListener((e, t, n) => {
  ...
  try {
    if ((e == null ? void 0 : e.type) === "ACTIVITY_UPDATE") {
      const a = e.payload;
      return t.tab && (a.tabId = t.tab.id, a.windowId = t.tab.windowId), R(a).then(() => p()).catch((i) => console.error("AutoEOD: failed to enqueue payload", i)), n({ status: "queued" }), !0;
    }
...
chrome.alarms.onAlarm.addListener((e) => {
  try {
    e.name === "retrySync" ? p() : e.name === "fetchSettings" && N();
  } catch (t) {
...
async function U() {
  try {
    await C() ? await N() : y("!", "#71717a"), await p();
  } catch (e) {
